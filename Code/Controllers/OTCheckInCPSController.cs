...
public class OTCheckInCPSController : Controller
{

    private readonly INotyfService _notyfService;

    public OTCheckInCPSController(INotyfService notyfService)
    {
    ...
    public async Task<IActionResult> Index(string _factory, string _department, DateTime _startDate, DateTime _endDate, string docNumber)
    {
        ...
        var dataPlant = await NetworkService.getPlantCPS(); //โรงงาน
        ViewData["PlantList"] = new SelectList
        (dataPlant.data, "plantid", "plantname", _factory);


        var dataFD = await NetworkService.getFac_DepCPS(); //แผนก
        ...
        {
            var resultListOT = await NetworkService.getListOTByDateCPS(sDate.ToString("yyyy-MM-dd"), eDate.ToString("yyyy-MM-dd"));

            foreach (var item in resultListOT.data)
            {
                var dataEmployee = await NetworkService.getOtByDocCPS(item.headerQTDoc);

                var dataOTByDoc = await NetworkService.getOtByDocCPS(item.headerQTDoc);
        ...
        {
            var resultListOT = await NetworkService.getListOTCPS(_factory, _department, sDate.ToString("yyyy-MM-dd"), eDate.ToString("yyyy-MM-dd"));
